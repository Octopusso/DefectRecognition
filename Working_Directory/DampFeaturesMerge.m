...
% Define regex pattern for G[1-9]_P5_case ending in 3 (e.g., case43)
pattern = '^G[1-9]_P5_case\d*3\.mat$';
...
% Step 2: Read and stack DampFeaturesTable
...
for i = 1:length(trainingFiles)
    ...
    data = load(fpath);
    if isfield(data, 'DampFeaturesTable')
        allTables{end+1} = data.DampFeaturesTable;
    else
        warning('No DampFeaturesTable in file: %s', trainingFiles(i).name);
    end
end

% Step 3: Combine and save as DampData.csv
if ~isempty(allTables)
    fullTable = vertcat(allTables{:});
    outPath = fullfile(dstFolder, 'DampData.csv');
    writetable(fullTable, outPath);
    fprintf('✅ DampData saved to %s\n', outPath);
else
    warning('⚠️ No DampFeaturesTable variables found to combine.');
end
